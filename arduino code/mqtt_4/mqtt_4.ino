...
#define MCUNAME "arduino1"    // nome directory mqtt primario
...
byte mac[]    = {  0xDE, 0xED, 0xBA, 0xFE, 0xFE, 0xED };      // imposta il mac address della LAN
IPAddress iplan (192, 168, 1, 104);                           // imposta l'indirizzo IP della LAN
...
String datainput = "FFFFFFFFFFFFFFFF";
String pinstatus = "FFFFFFFFFFFFFFFF";
uint8_t delivery = 0;
uint8_t fail = 0;
...
void setup()
{
  ...
  GPIOchip3.begin(B010);  
  GPIOchip4.begin(B011); 
  GPIOchip1.init();
  GPIOchip2.init(); 
  GPIOchip3.init();
  GPIOchip4.init(); 
  ...
  GPIOchip3.inputOutputMask(65535);
  GPIOchip3.internalPullupMask(65535);
  GPIOchip4.inputOutputMask(65535);
  GPIOchip4.internalPullupMask(65535);
...
void loop()
{
  ...
  pinstatus = String(GPIOchip3.digitalWordRead(), HEX) + pinstatus;
  while (pinstatus.length() < 12) pinstatus = "0" + pinstatus;
  pinstatus = String(GPIOchip4.digitalWordRead(), HEX) + pinstatus;
  while (pinstatus.length() < 16) pinstatus = "0" + pinstatus;
